...
export function Icon(props: IconProps): JSX.Element {
  const isFilledReadIcon = () => props.name === "read";
  const path = createMemo(() => renderIconPath(props.name));

  return (
    <svg
      aria-hidden="true"
      data-rtl-flip={isFilledReadIcon() ? "" : undefined}
      fill={isFilledReadIcon() ? "currentColor" : "none"}
      height={props.size ?? 18}
      stroke={isFilledReadIcon() ? "none" : "currentColor"}
    ...
    case "terminal":
      return (
        <>
          <path d="m7 9 3 3-3 3" />
          <path d="M13 15h4" />
          <rect width="20" height="16" x="2" y="4" rx="2.25" />
        </>
      );
    case "anchor":
      return (
