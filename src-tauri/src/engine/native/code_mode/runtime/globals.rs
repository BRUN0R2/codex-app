...
use super::callbacks::{
    audio_callback, clear_timeout_callback, exit_callback, generated_image_callback,
    image_callback, load_callback, notify_callback, set_timeout_callback, store_callback,
    text_callback, tool_callback, yield_control_callback,
};
...
fn build_tools_object<'s>(
    ...
    for (index, tool) in enabled_tools.iter().enumerate() {
        let name = v8::String::new(scope, &tool.name)
            .ok_or_else(|| "failed to allocate nested tool name".to_string())?;
        let function = tool_function(scope, index)?;
        if tools.set(scope, name.into(), function.into()) != Some(true) {
            return Err(format!("failed to install nested tool `{}`", tool.name));
        }
    }
    Ok(tools)
}
...
fn build_all_tools_value<'s>(
    ...
    for (index, tool) in enabled_tools.iter().enumerate() {
        ...
        if item.set(scope, name_key.into(), name.into()) != Some(true)
            ...
            || array.set_index(scope, index as u32, item.into()) != Some(true)
        {
            return Err("failed to build ALL_TOOLS metadata".into());
        }
    }
    Ok(array.into())
}
...
{
    let data = v8::String::new(scope, name)
        .ok_or_else(|| format!("failed to allocate helper `{name}`"))?;
    v8::FunctionTemplate::builder(callback)
        .data(data.into())
        .build(scope)
        .get_function(scope)
        .ok_or_else(|| format!("failed to create helper `{name}`"))
}
...
fn tool_function<'s>(
    ...
    v8::FunctionTemplate::builder(tool_callback)
        .data(data.into())
        .build(scope)
        .get_function(scope)
        .ok_or_else(|| "failed to create nested tool function".to_string())
}
