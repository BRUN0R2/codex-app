...
const MODEL_OVERRIDE_HINT: &str = "Full-history forks (`fork_turns` set to `null` or `\"all\"`) inherit the parent model and reasoning effort and do not accept overrides. Set `model` and `reasoning_effort` to `null` unless an override was explicitly requested by the user, applicable `AGENTS.md` instructions, or skill instructions; when overriding them, set `fork_turns` to `\"none\"` or a positive integer string.";
